...
doDownloadAur() {
  [ "$AUR_TOOL" == "yaourt" ] && doDownloadYaourt $*
  [ "$AUR_TOOL" == "yay" ] && doDownloadYay $*
}

doSuDownloadAur() {
  doSuSudo "$AUR_TOOL_USERNAME" suDownloadAur
}
...
doInstallAur() {
  [ "$AUR_TOOL" == "yaourt" ] && doInstallYaourt $*
  [ "$AUR_TOOL" == "yay" ] && doInstallYay $*
}

doSuInstallAur() {
  doSuSudo "$AUR_TOOL_USERNAME" suInstallAur
}
...
doAur() {
  [ "$AUR_TOOL" == "yaourt" ] && doYaourt $*
  [ "$AUR_TOOL" == "yay" ] && doYay $*
}

doSuAur() {
  doSuSudo "$AUR_TOOL_USERNAME" suAur $*
}
...
doX11InstallUbuntuFontRendering() {
  ...
  doSuAur cairo-ubuntu
  pacman -Rdd --noconfirm cairo
  doSuAur cairo-ubuntu

  pacman -Rdd --noconfirm freetype2
  doSuAur freetype2-ubuntu

  pacman -Rdd --noconfirm fontconfig
  doSuAur fontconfig-ubuntu
}
...
case "$INSTALL_TARGET" in
  ...
  chroot)
    ...
    if [ "$AUR_TOOL_DOWNLOAD" == "yes" ]; then
      doCopyToSu "$AUR_TOOL_USERNAME"
      doSuDownloadAur

      [ "$AUR_TOOL_INSTALL" == "yes" ] && doSuInstallAur
    fi
    ...
    if [ "$INSTALL_X11" == "yes" ]; then
      ...
      [ "$X11_INSTALL_I3" == "yes" ] && doX11Installi3
    ...
    if [ "$INSTALL_REMOVE_FROM_SU" == "yes" ]; then
      ...
      [ "$AUR_TOOL_INSTALL" == "yes" ] && doRemoveFromSu "$AUR_TOOL_USERNAME"
    fi
